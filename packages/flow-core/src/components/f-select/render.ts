...
export default function render(this: FSelect) {
	...
	if (this.width === "fill-container") {
		...
		this.style.width = this.width ? `${this.width}px` : "100%";
	}
	/**
	 ...
	 */
	const prefixAppend = html`<f-div direction="column" style="flex:1 1 auto">
		<div class="f-select-prefix">
		...
		>
			...
			>
				...
				<div
					...
					id="f-select-options"
					style="${this.applyOptionsStyle(this.openDropdown ? this.offsetWidth : 200)}"
					size=${ifDefined(this.size)}
				>
...
export function renderSingleSelection(this: FSelect, option: FSelectSingleOption) {
	...
	return html`<f-div padding="none" width="100%"> ${getTemplate()} </f-div>`;
}
...
export function renderMultipleSelectionTag(this: FSelect, option: FSelectSingleOption) {
	...
	const getTemplate = () => {
		return this.optionTemplate
			? html`<f-div variant="curved" gap="small" state="secondary" padding="small"
